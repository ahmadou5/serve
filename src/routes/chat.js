...
//gets chat and contacts
router.get('chats-and-contacts', (req, res) => {
  ...
  res.send(200, { chatsContacts, contacts: contactsToShow, profileUser: profileUserData})
})
//return user profile
router.get('/profile-user', (req, res) => {
  res.send(200, data.profileUser)
})
//return single chat
router.get('/get-chat', (req,res) => {
  ...
  res.send(200, {chat, contact})
})
// add new chat message on chat
router.post('/send-msg', (req, res) => {
  const { obj } = req.body
  const activeChat = data.chats.find(chat => chat.id === obj.contact.id)
  ...
  if(activeChat === undefined) {
    ...
    activeChat = data.chats[data.chatslength - 1]
  } else {
