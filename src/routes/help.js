...
router.get('/landing' , (req , res) => {
    ...
    res.send(200 , {allArticles, categories : data.categories, popularArticles: data.popularArticles, keepLearning: data.keepLearning})
})
...
router.get('/article', (req, res) => {
    ...
    const { articles, category , subCategories} = req.query
    const activeCategory = data.categories.filter(item => item.slug === category)[0]
    const activeSubCategory = activeCategory.subCategories.filter(item => item.slug === subCategories)[0] || activeCategory.subCategories[0]
    const activeArticles = activeSubCategory.articles.filter(item => item.slug === articles)[0]

    res.send(200 , {activeArticles, activeCategory , activeSubCategory ,categories: data.categories, articles: activeSubCategory.articles})
})
