...
router.get('/all-emails', (req, res) => {
    res.send(200, {emails : data.emails , length : data.emails.length()})
})
// get emails
router.get('/emails', (req, res) => {
    ...
    res.send(200, { emails: filteredData , emailsMeta })

})
//update email label
router.post('/update-email-label', (req, res) => {
    const { emailId, label } = req.body
    const updateMailLabels = (email) => {
        const labelIndex = email.label.indexOf(label)
        if (labelIndex === -1) email.labels.push(label)
        else email.labels.splice(labelIndex, 1)
    }
    data.emails.forEach(email => {
        if (emailId.includes(email.id)) updateMailLabels(email)
    })
...
router.get('/paginate-email', (req,res) => {
  ...
  const currentEmailIndex = paramsFilteredMails.findIndex(em => em.id === emailId)
  ...
  res.send( newEmail ? [200, newEmail] : 404 )
})
